...
@pytest.mark.parametrize(
    "transaction_main, expected",
    [
        (
            transaction_main,
            {
                "closed": "customLong-closed-1733172624209",
                "open": "customLong-open-1733172624209",
            },
        ),
    ],
)
def test_get_custom_label_oto(transaction_main, expected):
...
@pytest.mark.parametrize(
    "transaction_main, transaction_secondary, expected",
    [
        (
            transaction_main,
            transaction_secondary,
            {
                ...
                "entry_price": 90000.0,
                "size": 10.0,
                "label": "customLong-open-1733172624209",
                "side": "buy",
                "otoco_config": [
                    {
                        "amount": 10.0,
                        "direction": "sell",
                        "type": "limit",
                        "label": "customLong-closed-1733172624209",
                        "price": 100000,
                        "time_in_force": "good_til_cancelled",
                        "post_only": True,
                    }
...
def test_labelling_unlabelled_order_oto(
    ...
    result = labelling_unlabelled_order_oto(transaction_main, transaction_secondary)
