...
income_keys = [
    ...
    'enterpriseToRevenue',
    'enterpriseToEbitda',
    'trailingPegRatio' 'earningsQuarterlyGrowth',
    'earningsGrowth',
    'revenueGrowth',
]
...
cashflow_keys = ['freeCashflow', 'operatingCashflow']
...
if one_day.button('1d', use_container_width=True):
    selected_horizon = '1d'
    candlestick_dataframe = df_for_candlestick(
        selected_horizon, selected_ticker
    )
    candlestick = plot_candlestick(
        selected_ticker, selected_horizon, candlestick_dataframe
    )
...
predictions = forecast(
    forecast_model, feature_transform, target, forecast_days=30
)
predictions_series = create_predictions_series(forecast_dataframe, predictions)
forecast_plot = plot_forecast(
    selected_ticker, forecast_dataframe, predictions_series
)
