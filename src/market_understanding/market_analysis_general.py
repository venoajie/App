...
class Options:
    ...
    def time_to_expiration(self) -> float:
        ...
        transaction_time_utc = formula.convert_time_to_utc(
...
def is_contango_or_backwardation(
    ...
    return {
        ...
        'margin_pct': []
        if futures_price == []
        else margin / spot_or_perpetual_price,
...
def fetch_and_combine_funding_rate_with_heatmap_result(dataframe):
    ...
    try:
        ...
        dataframe['nextFundingRate'] = [
            (
                fetch_next_funding.fetch_next_funding_coin((o))[
                    'nextFundingRate'
                ]
            )
...
def fetch_correlation_from_sqlite_(threshold) -> str:
    ...
    correl_above_threshold = [
        o for o in [o for o in correl if o['correl'] > threshold]
    ]
    correl_below_threshold = [
        o for o in [o for o in correl if o['correl'] < threshold]
    ]
    con.close()
    ...
    return {
        ...
        'above_threshold_rate_positive': [
            o
            for o in [
                o for o in correl_above_threshold if o['nextFundingRate'] > 0
            ]
        ],
...
def heatmap_and_funding_rate_(
    ...
    try:
        ...
        for symbol in symbols:
            symbol_underlying = (
                formula.transform_underlying_from_or_to_perpetual(
                    symbol, 'underlying'
                )
            )
            ...
            coeffs[symbol] = ohlc_mid_main.corr(
                ohlc_df['close'], min_periods=300
            )
...
if __name__ == '__main__':
    ...
    strike = 23400  # Strike
    time_to_expiration = (
        '2022-08-20T00:00:00+00:00'  # (Annualized) time-to-expiration
    )
    interest = 0  #
