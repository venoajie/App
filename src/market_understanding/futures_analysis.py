...
def compute_remaining_active_hours_before_expiration(
    future_expiration: int,
) -> float:
    """
    ...
    """
    return time_modification.time_delta_between_two_times(
        'unix-ms', get_now_unix_time(), future_expiration
    )['hours']
...
def get_futures_market_expectation(margin: float) -> str:
    ...
    return (
        'contango'
        if margin > 0
        else ('backwardation' if margin < 0 else 'neutral')
    )
...
def get_margin_and_ratio(mark_price: float, index_price: float) -> dict:
    """ """
    margin = mark_price - index_price

    return {'margin': margin, 'ratio_price_to_index': margin / index_price}


def combining_individual_futures_analysis(
    index_price: float, future: dict, ticker: dict
) -> dict:
    """ """
    ...
    try:
        ...
        margin_and_ratio = get_margin_and_ratio(mark_price, index_price)
        ...
        dicttemp = {
            ...
            'market_expectation': get_futures_market_expectation(
                margin_and_ratio['margin']
            ),
            'mark_price': mark_price,
            'margin_usd': margin_and_ratio['margin'],
            'ratio_price_to_index': margin_and_ratio['ratio_price_to_index'],
            'remaining_active_time_in_hours': round(
                compute_remaining_active_hours_before_expiration(
                    expiration_time
                ),
                2,
            ),
