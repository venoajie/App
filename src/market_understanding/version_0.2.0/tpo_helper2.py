...
def abc(session_hr=6.5, freq=30):
    ...
    if alimit > 52:
        alphabets = Aa * int(
            (np.ceil((alimit - 52) / 52)) + 1
        )  # if bar frequency is less than 30 minutes then multiply list
    else:
...
def tpo(dft_rs, freq=30, ticksize=10, style='tpo', session_hr=6.5):
    if len(dft_rs) > int(60 / freq):
        ...
        df_va['cum_tpo'] = np.where(
            df_va.bel > df_va.abv, df_va.bel, df_va.cum_tpo
        )

        df_va['cum_tpo'] = np.where(
            df_va.abv == df_va.bel, df_va.abv + df_va.bel, df_va.cum_tpo
        )
        ...
        exhandle_index = np.where(
            tpoval <= 2, tpoval.index, None
        )  # get index where TPOs are 2
...
def get_dayrank(dist_df, mean_val):
    ...
    dist_df['daytype'] = np.where(
        np.logical_and(
            dist_df.distr >= dismean, dist_df.distr < dismean + dissig
        ),
        'Trend Distribution Day',
        '',
    )

    dist_df['daytype_num'] = np.where(
        np.logical_and(
            dist_df.distr >= dismean, dist_df.distr < dismean + dissig
        ),
        3,
    ...
    dist_df['daytype'] = np.where(
        dist_df.distr < dismean - dissig, 'Neutral Day', dist_df['daytype']
    )

    dist_df['daytype_num'] = np.where(
        dist_df.distr < dismean - dissig, 1, dist_df['daytype_num']
    )

    dist_df['daytype'] = np.where(
        dist_df.distr > dismean + dissig, 'Trend Day', dist_df['daytype']
    )
    dist_df['daytype_num'] = np.where(
        dist_df.distr > dismean + dissig, 4, dist_df['daytype_num']
    )
    dist_df['daytype_num'] = np.where(
        dist_df.close >= dist_df.poclist,
        dist_df.daytype_num * 1,
        dist_df.daytype_num * -1,
    )  # assign signs as per bias
    ...
    dist_df['vold_zscore'] = (
        dist_df.volumed - vol_mean
    ) / dist_df.volumed.std(ddof=0)
    dist_df['rfd_zscore'] = (abs(dist_df.rfd) - rf_mean) / abs(
        dist_df.rfd
    ).std(ddof=0)
    a, b = 1, 4
    ...
    dist_df['Volume_Factor'] = np.where(
        dist_df.close >= dist_df.poclist,
        dist_df.norm_volume * 1,
        dist_df.norm_volume * -1,
    )
    dist_df['Rotation_Factor'] = np.where(
        dist_df.rfd >= 0, dist_df.norm_rf * 1, dist_df.norm_rf * -1
    )
    ...
    ranking_df['H_vs_yH'] = np.where(
        ranking_df.highd >= ranking_df.highd.shift(), 1, -1
    )
    ranking_df['L_vs_yL'] = np.where(
        ranking_df.lowd >= ranking_df.lowd.shift(), 1, -1
    )
    ...
    ranking_df['CL<VAL'] = np.where(
        ranking_df.close < ranking_df.vallist, -2, 0
    )
    ranking_df['CL>=VAH'] = np.where(
        ranking_df.close >= ranking_df.vahlist, 2, 0
    )

    ranking_df['power1'] = 100 * (
        (
            ranking_df.VAH_vs_yVAH
            + ranking_df.VAL_vs_yVAL
            + ranking_df.POC_vs_yPOC
            + ranking_df.H_vs_yH
            + ranking_df.L_vs_yL
            + ranking_df['Close_vs_yCL']
            + ranking_df['CL>POC<VAH']
            + ranking_df['CL<poc>val']
            + ranking_df.Single_Prints
            + ranking_df['CL<VAL']
            + ranking_df['CL>=VAH']
...
def get_ibrank(mean_val, ranking):
    ...
    ibrankdf['ibrf_zscore'] = (abs(ibrankdf.ibrf) - ibrf_mean) / abs(
        ibrankdf.ibrf
    ).std(ddof=0)
    a, b = 1, 4
    e, f = ibrankdf.ibrf_zscore.min(), ibrankdf.ibrf_zscore.max()
    ibrankdf['ibnorm_rf'] = (ibrankdf.ibrf_zscore - e) / (f - e) * (b - a) + a
    ibrankdf['IB_RotationFactor'] = np.where(
        ibrankdf.ibrf >= 0, ibrankdf.ibnorm_rf * 1, ibrankdf.ibnorm_rf * -1
    )
    ibrankdf['IB_RotationFactor'] = ibrankdf['IB_RotationFactor'].round(2)
    ibrankdf['ibvol_zscore'] = (
        ibrankdf.ibvol - ibvol_mean
    ) / ibrankdf.ibvol.std(ddof=0)
    a, b = 1, 4
    g, h = ibrankdf.ibvol_zscore.min(), ibrankdf.ibvol_zscore.max()
    ibrankdf['ibnorm_vol'] = (ibrankdf.ibvol_zscore - g) / (h - g) * (
        b - a
    ) + a
    ibrankdf['IB_Volume_factor'] = np.where(
        ibrankdf.close >= ibrankdf.poclist,
    ...
    ibrankdf['IB>yPOC<yVAH'] = np.where(
        np.logical_and(
            ibrankdf.ibmid >= ibrankdf.poclist.shift(),
            ibrankdf.ibmid < ibrankdf.vahlist.shift(),
        ),
        1,
        0,
    )
    ibrankdf['IB<yPOC>yVAL'] = np.where(
        np.logical_and(
            ibrankdf.ibmid >= ibrankdf.vallist.shift(),
            ibrankdf.ibmid < ibrankdf.poclist.shift(),
        ),
        -1,
        0,
    ...
    ibrankdf['IB>yVAH<yH'] = np.where(
        np.logical_and(
            ibrankdf.ibmid >= ibrankdf.vahlist.shift(),
            ibrankdf.ibmid < ibrankdf.highd.shift(),
        ),
        2,
        0,
    )
    ibrankdf['IB<yVAL>yL'] = np.where(
        np.logical_and(
            ibrankdf.ibmid < ibrankdf.vallist.shift(),
            ibrankdf.ibmid > ibrankdf.lowd.shift(),
        ),
        -2,
        0,
    )
    ibrankdf['IB>yH'] = np.where(
        ibrankdf.ibmid >= ibrankdf.highd.shift(), 4, 0
    )
    ibrankdf['IB<yL'] = np.where(ibrankdf.ibmid < ibrankdf.lowd.shift(), -4, 0)

    ibrankdf['ibpower1'] = 100 * (
        (
            ibrankdf['IB>yPOC<yVAH']
            + ibrankdf['IB<yPOC>yVAL']
            + ibrankdf['IB>yVAH<yH']
            + ibrankdf['IB<yVAL>yL']
            + ibrankdf['IB>yH']
            + ibrankdf['IB<yL']
            + ibrankdf['IB_Volume_factor']
            + ibrankdf['IB_RotationFactor']
        )
        / 8
    )
    ...
    ibrankdf['IB_power'] = (abs(ibrankdf.ibpower1) - ibmin) / (
        ibmax - ibmin
    ) * (bb - aa) + aa
...
def get_mean(dfhist, avglen=30, freq=30):
    ...
    rf30 = (
        abs((dfhistd['rf'])).rolling(avglen).mean()
    )  # it is abs mean to get meaningful value to compare daily values
    ...
    all_val = dict(
        ...
        rf_mean=rf_mean,
        volib_mean=volib_mean,
        ibrf_mean=ibrf_mean,
        session_hr=session_hr,
    )
