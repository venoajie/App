...
def get_dayrank(dist_df, mean_val):
    ...
    dist_df["daytype"] = np.where(
        np.logical_and(dist_df.distr >= dismean, dist_df.distr < dismean + dissig),
...
def get_ibrank(mean_val, ranking):
    ...
    ibrankdf["ibrf_zscore"] = (abs(ibrankdf.ibrf) - ibrf_mean) / abs(ibrankdf.ibrf).std(
        ddof=0
    )
    a, b = 1, 4
    ...
    ibrankdf["ibvol_zscore"] = (ibrankdf.ibvol - ibvol_mean) / ibrankdf.ibvol.std(
        ddof=0
    )
    a, b = 1, 4
    ...
    ibrankdf["IB_power"] = (abs(ibrankdf.ibpower1) - ibmin) / (ibmax - ibmin) * (
        bb - aa
    ) + aa
