...
def check_duplicate_elements(file_name: str) -> None:
    ...
    free_from_duplicates_data = string_modification.remove_redundant_elements(
        data_from_db
    )
...
def dump_data_as_list(
    file_name: str, data: dict, check_duplicates: bool = False
) -> None:
    """ """
        ...
        try:
            ...
            if data != []:
                ...
                if isinstance(data, list):
                    free_from_none_data = [
                        o for o in data if isinstance(o, dict)
                    ]
...
def append_data(
    file_name_pkl: str, data: dict, check_duplicates: bool = False
) -> None:
    """
    ...
    dump_data_as_list(file_name_pkl, combined_data, check_duplicates)
...
def append_and_replace_items(
    ...
    if isinstance(data_from_db, list):
        try:
            ...
            print(
                f'except because of dict type. WEIRD. RECHECK) {data_from_db}'
            )
            data_list = list(data_from_db)

    dump_data_as_list(file_name_pkl, data_list, check_duplicates)


def append_and_replace_items_based_on_qty(
    file_name_pkl: str,
    data: dict,
    max_qty: int,
    ...
    if 'change_id' in data_list:
        ...
        if len_tick_data_ordBook > max_qty:
            ...
            result: list = [
                o
                for o in data_from_db
                if o['timestamp'] not in filtered_timestamps
            ]
    ...
    if 'params' in data_list:
        ...
        if len_tick_data_ohlc > max_qty:
            ...
            result: list = [
                o for o in data_from_db if o['tick'] not in filtered_timestamps
            ]

            dump_data_as_list(file_name_pkl, result, check_duplicates)


def append_and_replace_items_based_on_time_expiration(
    file_name_pkl: str,
    data: dict,
    time_expiration: int,
    check_duplicates: bool = False,
    ...
    if 'with_rebates' in data_list and False:
        result: list = [
            o
            for o in data
            if o['remaining_active_time_in_hours'] == one_hour_ago
        ]
        dump_data_as_list(file_name_pkl, result, check_duplicates)
